...
<head>
<meta charset="UTF-8">
<title>${esc(inv.bill)} - Clean N Clear</title>
<style>
@page{size:A5 portrait;margin:5mm}
*{box-sizing:border-box}
body{font-family:Arial,Helvetica,sans-serif;color:#111;margin:0;font-size:10.5px}
.invoice{width:100%;border:1px solid #222;min-height:100%}
.header{padding:9px 10px 6px;border-bottom:1px solid #222;display:flex;justify-content:space-between;gap:10px}
.business h1{font-size:20px;margin:0 0 3px;text-transform:uppercase}
.business p{margin:1px 0;font-size:9px}
.invoice-title{text-align:right;min-width:105px}
.invoice-title h2{font-size:19px;margin:0 0 7px}
.invoice-title p{margin:2px 0;font-size:10px}
.party{display:grid;grid-template-columns:1fr 1fr;border-bottom:1px solid #222}
.party-box{padding:7px 10px;min-height:55px}
.party-box+ .party-box{border-left:1px solid #222}
.party-label{font-weight:bold;font-size:9px;text-transform:uppercase;margin-bottom:3px}
.party-name{font-weight:bold;font-size:11px}
table{width:100%;border-collapse:collapse}
th,td{border-bottom:1px solid #999;border-right:1px solid #999;padding:5px 5px}
th:last-child,td:last-child{border-right:0}
thead th{border-bottom:1px solid #222;background:#f5f5f5;font-size:9.5px}
.sn{width:24px;text-align:center}.num{text-align:right;white-space:nowrap}
.total-row td{border-top:1px solid #222;border-bottom:0;font-weight:bold}
.grand{font-size:12px}
.footer{padding:7px 10px;border-top:1px solid #222;display:flex;justify-content:space-between;gap:8px}
.note{font-size:8.5px}
.sign{text-align:right;font-weight:bold}
@media print{body{margin:0}.invoice{border:1px solid #222}}
</style>
</head>
<body>
<div class="invoice">
  <div class="header">
    <div class="business">
      ...
      <p><b>Mobile:</b> ${esc(BUSINESS.mobile)}</p>
    </div>
    <div class="invoice-title">
      <h2>INVOICE</h2>
      <p><b>Invoice No.:</b> ${esc(inv.bill)}</p>
      <p><b>Date:</b> ${esc(inv.date)}</p>
    </div>
  </div>
  ...
  <div class="party">
    <div class="party-box">
      ...
      <div class="party-name">${esc(inv.customer||"Cash")}</div>
    </div>
    <div class="party-box">
      <div class="party-label">Payment</div>
      <div>${inv.payment||"Cash / As received"}</div>
    </div>
  </div>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr class="total-row">
        ...
        <td class="num grand">${money(inv.amount)}</td>
      </tr>
    </tbody>
  </table>

  <div class="footer">
    <div class="note">Thank you for your business.<br>GST not charged.</div>
    <div class="sign">For ${esc(BUSINESS.name)}<br><br>Authorized Signatory</div>
  </div>
</div>
...
function render(){
 ...
 const stock=db.products.map(p=>{const purchased=db.purchases.filter(x=>x.productId===p.id || x.product===p.name).reduce((s,x)=>s+(+x.qty||0),0),sold=db.sales.flatMap(x=>x.items||[]).filter(x=>x.productId===p.id || x.product===p.name).reduce((s,x)=>s+(+x.qty||0),0);return{p,purchased,sold,current:(+p.openingStock||0)+purchased-sold}});
